        ...
        test('should be able to add an image via a base64 encoded string', async () => {
            try {
                ...
                await slide.addImage(
                    ...
                    { x: 350, y: 250, cx: 25, cy: 25 }
                );

                await slide.addImage(
                    {
                        downloadUrl: 'https://www.mcdonalds.com/content/dam/usa/logo/m_logo.png',
                    },
                    { x: 350, y: 350, cx: 25, cy: 25 }
                );

                await presentation.save(`${tmpDir}/presentation-new-add-base64-image.pptx`);
