...
let { hasOwnProperty } = require('../helpers/utils');
...
class PowerPointFactory {
    ...
    async loadFromRawFileData(data) {
        ...
        for (let key in zip.files) {
            if (hasOwnProperty(zip.files, key)) {
    ...
    clearContent() {
        for (let key in this.content) {
            if (hasOwnProperty(this.content, key)) {
    ...
    extractObjectsFromContent(content) {
        ...
        for (let slideName in slideInformation) {
            if (hasOwnProperty(slideInformation, slideName)) {
    ...
    getSlide(slideName) {
        if (!hasOwnProperty(this.slides, slideName)) throw new Error(`Slide name doesn't exist in PowerPointFactory.getSlide(): '${slideName}' `);
    ...
    async addImageFromRemoteUrl(slide, image) {
        const response = await fetch(image.downloadUrl, { signal: AbortSignal.timeout(30000) });
        image.source = await response.arrayBuffer();
    ...
    rebuild() {
        ...
        for (let slideName in this.slides) {
            if (hasOwnProperty(this.slides, slideName)) {
