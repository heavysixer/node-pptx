...
class PowerPointFactory {
    ...
    setSlideSize(width, height) {
        this.pptFactory.setSlideSize(width, height);
    }
    ...
    addChart(slide, data, options = {}) {
        let chartName = `chart${Object.keys(this.charts).length + 1}`;
        let newChartContentBlock = this.pptFactory.addChart(slide, chartName, data, options);
        ...
        this.contentTypeFactory.addDefaultType(`xlsx`, 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');

        // TODO: see if you can make this function async so you won't need a callback
        ExcelHelper.createWorkbook(data, function(err, workbookJSZip) {
            let workbookContentZipBinary = workbookJSZip.generate({ type: 'arraybuffer' });
            let numWorksheets = this.getWorksheetCount();
            let worksheetName = `Microsoft_Excel_Sheet${numWorksheets + 1}.xlsx`;

            // TODO: refactor this stuff into their own factories...
