...
class PresentationFactory {
    ...
    setLayout(layout) {
        ...
        if (originalCx !== layout.width || originalCy !== layout.height) {
            let slideWidthPixels = PptxUnitHelper.toPixels(layout.width);
            let slideHeightPixels = PptxUnitHelper.toPixels(layout.height);
            let size = this.getSlideNumberShapeSizeFromLayout('slideLayout1');

            size.cx = PptxUnitHelper.toPixels(size.cx);
            size.cy = PptxUnitHelper.toPixels(size.cy);

            let newX = slideWidthPixels - size.cx - 25;
            let newY = slideHeightPixels - size.cy;

            // console.log('slide width pixels = ', slideWidthPixels);
            // console.log('slide height pixels = ', slideHeightPixels);
            // console.log('newX = ', newX);
            // console.log('newY = ', newY);

            newX = PptxUnitHelper.fromPixels(newX);
            newY = PptxUnitHelper.fromPixels(newY);
    ...
    moveSlideNumberOnLayoutTemplate(layoutName, x, y) {
        ...
        if (slideNumberNode !== undefined) {
            ...
            //console.log('tree = ', slideNumberNode);
        }
    }
