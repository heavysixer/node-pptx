...
class SlideFactory {
    ...
    addText(slide, text, options = {}) {
        ...
        newTextBlock['p:txBody'][0]['a:p'][0]['a:endParaRPr'] = [{ $: { lang: 'en-US' } }]; // this MUST go last!
    ...
    addShape(slide, type, options = {}) {
        ...
        newShapeBlock['p:txBody'][0]['a:p'][0]['a:endParaRPr'] = [{ $: { lang: 'en-US' } }]; // this MUST go last!
    ...
    addChart(slide, chartName, data, options = {}) {
        let slideKey = `ppt/slides/${slide.name}.xml`;
        let chartKey = `ppt/charts/${chartName}.xml`;

        // TODO: put in x/y/cx/y defaults

        // TODO: once tested, reorder these lines below...
        let newGraphicFrameBlock = PptFactoryHelper.createBaseChartFrameBlock(
            PptxUnitHelper.fromPixels(options.x),
            PptxUnitHelper.fromPixels(options.y),
            PptxUnitHelper.fromPixels(options.cx),
            PptxUnitHelper.fromPixels(options.cy)
        );

        // TODO: I don't think this block is needed, try and take it out when done testing...
        newGraphicFrameBlock['p:graphicFrame'][0]['p:nvGraphicFramePr'][0]['p:nvPr'][0]['p:extLst'][0]['p:ext'][0]['p14:modId'] = [
            {
                $: {
        ...
        let newChartSpaceBlock = PptFactoryHelper.createBaseChartSpaceBlock();
        let chartSeries = { 'c:ser': data.data.map(this._ser, this) }; // TODO: "data.data"? rename "data" to something like chartInfo

        newChartSpaceBlock['c:chartSpace']['c:chart'][0]['c:plotArea'][0]['c:barChart'][0]['c:ser'] = chartSeries['c:ser'];
    ...
    _ser(serie, i) {
        let rc2a = this._rowColToSheetAddress; // shortcut
        ...
        let ser = XmlNode()
            ...
            .addChild(
                'c:val',
                this._numRef(`Sheet1!${rc2a(2, 2 + i, true, true)}:${rc2a(2 + serie.labels.length - 1, 2 + i, true, true)}`, serie.values, 'General')
            );

        if (serie.color) {
            ser.addChild('c:spPr', XmlNode().addChild('a:solidFill', XmlNode().addChild('a:srgbClr', XmlNode().attr('val', serie.color))));
        } else if (serie.schemeColor) {
            ser.addChild('c:spPr', XmlNode().addChild('a:solidFill', XmlNode().addChild('a:schemeClr', XmlNode().attr('val', serie.schemeColor))));
        }

        return ser.el;
    }

    _strRef(region, stringArr) {
        let strRef = XmlNode().addChild(
            'c:strRef',
            XmlNode()
                .addChild('c:f', region)
    ...
    _strCache(stringArr) {
        let strRef = XmlNode().addChild('c:ptCount', XmlNode().attr('val', stringArr.length));
    ...
    _numRef(region, numArr, formatCode) {
        ...
        for (let i = 0; i < numArr.length; i++) {
            numCache.addChild(
                ...
                XmlNode()
                    .attr('idx', i)
                    .addChild('c:v', numArr[i].toString())
            );
        }

        let numRef = XmlNode().addChild(
            'c:numRef',
            XmlNode()
                .addChild('c:f', region)
    ...
    _rowColToSheetAddress(row, col, isRowAbsolute, isColAbsolute) {
        let address = '';
