...
class SlideFactory {
    ...
    addText(slide, text, options = {}) {
        ...
        if (options.font_face) {
            let fontAttributes = [{ $: { typeface: options.font_face, pitchFamily: '34', charset: '0' } }];
            let textRunProperties = newTextBlock['p:txBody'][0]['a:p'][0]['a:r'][0]['a:rPr'][0];

            textRunProperties['a:latin'] = fontAttributes;
            textRunProperties['a:cs'] = fontAttributes;

            endParagraphRun[0]['a:latin'] = fontAttributes;
            endParagraphRun[0]['a:cs'] = fontAttributes;
        }
