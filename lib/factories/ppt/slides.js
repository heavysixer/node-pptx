...
class SlideFactory {
    ...
    addSlide(slideName, layoutName) {
        ...
        //console.log('this.content = ', this.content[layoutKey]);
    ...
    addShape(slide, type, options = {}) {
        ...
        let shapeColor = options.color || '0000FF';
        ...
        newShapeBlock['p:spPr'][0]['a:solidFill'] = [{ 'a:srgbClr': [{ $: { val: shapeColor } }] }];
        newShapeBlock['p:txBody'][0]['a:bodyPr'] = [{ $: { anchor: 'ctr', rtlCol: '0' } }];
        newShapeBlock['p:txBody'][0]['a:p'] = [{ 'a:pPr': [{ $: { algn: 'ctr' } }] }];

        if (type.avLst) {
            newShapeBlock['p:spPr'][0]['a:prstGeom'][0]['a:avLst'] = [];

            for (let prop in type.avLst) {
                if (type.avLst.hasOwnProperty(prop)) {
                    newShapeBlock['p:spPr'][0]['a:prstGeom'][0]['a:avLst'].push({
                        'a:gd': [{ $: { name: 'adj', fmla: `val ${type.avLst[prop]}` } }],
                    });
                }
            }
        }
    ...
    addChart(slide, chartName, data, options = {}) {
        ...
        this.content[slideKey]['p:sld']['p:cSld'][0]['p:spTree'][0]['p:graphicFrame'] = newGraphicFrameBlock;

        let newChartSpaceBlock = PptFactoryHelper.createBaseChartSpaceBlock();
        let chartSeries = { 'c:ser': data.map(this._ser, this) };

        newChartSpaceBlock['c:chartSpace'][0]['c:chart'][0]['c:plotArea'][0]['c:barChart'][0]['c:ser'] = chartSeries;
