...
class SlideFactory {
    ...
    addImage(slide, imageFileName, rId, options = {}) {
        ...
        PptFactoryHelper.assignDefaults({ x: 0, y: 45, cx: 720, cy: 445 }, options);
    ...
    addText(slide, text, options = {}) {
        ...
        PptFactoryHelper.assignDefaults({ x: 0, y: 10, cx: 222, cy: 30 }, options);
    ...
    addShape(slide, type, options = {}) {
        ...
        PptFactoryHelper.assignDefaults({ x: 0, y: 0, cx: 222, cy: 30 }, options);
    ...
    addChart(slide, chartName, type, data, options = {}) {
        ...
        PptFactoryHelper.assignDefaults({ x: 0, y: 0, cx: 600, cy: 400 }, options);
    ...
    setBackgroundColor(slide, color) {
        ...
        if (slideContent['p:bg']) {
            if (!slideContent['p:bg'][0]['p:bgPr']) {
            ...
            for (let key in slideContent) {
                if (slideContent.hasOwnProperty(key)) {
                    existingNodes.push({ key: key, node: slideContent[key] });
                }
            }
            ...
            for (let key in slideContent) {
                if (slideContent.hasOwnProperty(key)) {
                    delete slideContent[key];
                }
            }

            // ...and finally add the <p:bg> node and existing content nodes back in.
            slideContent['p:bg'] = [{}];
            slideContent['p:bg'][0]['p:bgPr'] = [{}];
            ...
            for (let i = 0; i < existingNodes.length; i++) {
                slideContent[existingNodes[i].key] = existingNodes[i].node;
            }
        }
    }
}
