...
class PptFactory {
    ...
    async setBackgroundImage(slide, image) {
        let mediaName = `image-${uuidv4()}${path.extname(image.downloadUrl)}`;
        let rId = await this.slideFactory.addImageToSlideRelationship(slide, `../media/${mediaName}`);
        this.slideFactory.setBackgroundImage(slide, image, mediaName, rId);
    }
