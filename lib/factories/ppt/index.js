...
class PptFactory {
    ...
    addImage(slide, filePath, options = {}) {
        ...
        if (options.url) {
            ...
            if (options.url[0] !== '#') {
                ...
                // MUST contain the protocol prefix; so we'll put "http://" if the caller didn't specify it
                if (!options.url.startsWith('http')) {
                    options.url = `http://${options.url}`;
                }
            }

            options.rIdForHyperlink = this.slideFactory.addHyperlinkToSlideRelationship(slide, options.url);
        }
    ...
    addText(slide, text, options = {}) {
        if (options.url) {
            options.rId = this.slideFactory.addHyperlinkToSlideRelationship(slide, options.url);
        }
