...
class PptFactoryHelper {
    ...
    static validateColor(color, fallbackRgbColor) {
        ...
        if (!isRgb && !schemeColorValues.includes(color)) {
            ...
            return { isRgb: isRgb, color: fallbackRgbColor };
        }

        return { isRgb: isRgb, color: color };
    }
    ...
    static createSeriesDataBlock(data) {
        return {
            'c:ser': data.map(this.createSingleSeriesDataNode, this),
        };
    }
        ...
        if (options.textAlign) {
            ...
            if (alignment) {
                if (paragraphBlock['a:pPr'] === undefined) {
                    paragraphBlock['a:pPr'] = [
                        {
                            $: {},
                        },
                    ]; // create the node if it's not already there
                }
    ...
    static addLinePropertiesToBlock(block, lineProperties) {
        ...
        if (lineProperties.dashType) {
            block['a:ln'][0]['a:prstDash'] = [
                {
                    $: {
                        val: lineProperties.dashType,
                    },
                },
            ];
        }
