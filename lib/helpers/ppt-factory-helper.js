...
let { hasOwnProperty } = require('./utils');
...
class PptFactoryHelper {
    ...
    static createStrRefNode(region, stringArray) {
        let node = XmlNode().addChild(
            'c:strRef',
            XmlNode().addChild('c:f', region).addChild('c:strCache', PptFactoryHelper.createPtCountNode(stringArray))
        );
    ...
    static createNumRefNode(region, numArray, formatCode) {
        let numCache = XmlNode().addChild('c:formatCode', formatCode).addChild('c:ptCount', XmlNode().attr('val', numArray.length));

        for (let i = 0; i < numArray.length; i++) {
            numCache.addChild('c:pt', XmlNode().attr('idx', i).addChild('c:v', numArray[i].toString()));
        }

        let node = XmlNode().addChild('c:numRef', XmlNode().addChild('c:f', region).addChild('c:numCache', numCache));
    ...
    static convertTextPropertiesToOptionsAndMerge(textObject, options) {
        ...
        for (let prop in textObject) {
            if (hasOwnProperty(textObject, prop) && !['text', 'textSegments', 'options', 'x', 'y', 'cx', 'cy'].includes(prop)) {
        ...
        for (let prop in options) {
            if (hasOwnProperty(options, prop)) {
                ...
                if (!hasOwnProperty(textObject, prop)) {
