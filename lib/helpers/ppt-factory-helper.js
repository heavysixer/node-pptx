...
class PptFactoryHelper {
        ...
        if (options.textAlign) {
            ...
            if (alignment) {
                if (paragraphBlock['a:pPr'] === undefined) {
                    //paragraphBlock['a:pPr'] = [{ $: {} }]; // create the node if it's not already there
    ...
    static addTextValuesToBlock(block, textValues, options) {
        if (Array.isArray(textValues)) {
            let lines = textValues;
            block['a:p'] = [];
            ...
            for (let i = 0; i < lines.length; i++) {
                ...
                let paragraphBlock = (block['a:p'][i] = PptFactoryHelper.createEmptyParagraphPropertiesBlock()[0]);
                let bulletLvl0Margin = 228600;

                PptFactoryHelper.addParagraphPropertiesToBlock(paragraphBlock, options);

                if (i === 0) {
                    paragraphBlock['a:pPr'][0]['$'].marL = bulletLvl0Margin * (i + 1);
                    paragraphBlock['a:pPr'][0]['$'].indent = `-${bulletLvl0Margin}`;
                    paragraphBlock['a:pPr'][0]['a:buFont'] = [{ $: { typeface: '+mj-lt' } }];
                    paragraphBlock['a:pPr'][0]['a:buAutoNum'] = [{ $: { type: 'arabicParenR' } }];
                } else if (i === 1) {
                    paragraphBlock['a:pPr'][0]['$'].marL = bulletLvl0Margin * (i + 1);
                    paragraphBlock['a:pPr'][0]['$'].lvl = '1';
                    paragraphBlock['a:pPr'][0]['$'].indent = `-${bulletLvl0Margin}`;
                    paragraphBlock['a:pPr'][0]['a:buFont'] = [{ $: { typeface: '+mj-lt' } }];
                    paragraphBlock['a:pPr'][0]['a:buAutoNum'] = [{ $: { type: 'alphaLcParenR' } }];
                }

                block['a:p'][i]['a:r'] = [{ 'a:rPr': [{ $: { lang: 'en-US', smtClean: '0' } }], 'a:t': lineText }];

                let textRunPropertyBlock = block['a:p'][i]['a:r'][0]['a:rPr'][0];
                ...
                PptFactoryHelper.addBulletPointsToBlock(block['a:p'], lines[i], 0, options);
            }
            ...
            paragraphBlock['a:r'] = [{ 'a:rPr': [{ $: { lang: 'en-US', smtClean: '0' } }], 'a:t': textValues }];
    ...
    static addBulletPointsToBlock(masterPNode, textValue, indentLevel, options) {
        if (typeof textValue === 'string' || typeof textValue === 'number') {
            masterPNode.push({});

            let paragraphNodeIndex = masterPNode.length - 1;
            let paragraphBlock = (masterPNode[paragraphNodeIndex] = PptFactoryHelper.createEmptyParagraphPropertiesBlock()[0]);
            let bulletLvl0Margin = 228600;

            PptFactoryHelper.addParagraphPropertiesToBlock(paragraphBlock, options);

            paragraphBlock['a:pPr'][0]['$'].marL = bulletLvl0Margin * (indentLevel + 1);
            paragraphBlock['a:pPr'][0]['$'].indent = `-${bulletLvl0Margin}`;
            paragraphBlock['a:pPr'][0]['a:buFont'] = [{ $: { typeface: '+mj-lt' } }];
            ...
            masterPNode[paragraphNodeIndex]['a:r'] = [{ 'a:rPr': [{ $: { lang: 'en-US', smtClean: '0' } }], 'a:t': textValue }];

            let textRunPropertyBlock = masterPNode[paragraphNodeIndex]['a:r'][0]['a:rPr'][0];
            ...
            for (let i = 0; i < lines.length; i++) {
                PptFactoryHelper.addBulletPointsToBlock(masterPNode, lines[i], indentLevel + 1, options);
            }
        }
    }
}
