...
class PptFactoryHelper {
    ...
    static addBulletPointsToBlock(masterParagraphNode, textValue, indentLevel, options) {
        ...
        if (typeof textValue === 'string' || typeof textValue === 'number') {
            ...
            // masterParagraphNode[paragraphNodeIndex]['a:r'] = [{ 'a:rPr': [{ $: { lang: 'en-US', smtClean: '0' } }], 'a:t': textValue }];
            // let textRunPropertyBlock = masterParagraphNode[paragraphNodeIndex]['a:r'][0]['a:rPr'][0];
            //
            // PptFactoryHelper.setupTextRunPropertiesBlock(textRunPropertyBlock, options);
            ...
            if (textObject.text !== undefined) {
                ...
                // for (let segment of textObject.textSegments) {
                //     PptFactoryHelper.convertTextPropertiesToOptionsAndMerge(segment, options);
                //
                //     masterParagraphNode[paragraphNodeIndex]['a:r'].push({ 'a:rPr': [{ $: { lang: 'en-US', smtClean: '0' } }], 'a:t': segment.text });
                //     let textRunPropertyBlock = masterParagraphNode[paragraphNodeIndex]['a:r'][r]['a:rPr'][0];
                //
                //     PptFactoryHelper.setupTextRunPropertiesBlock(textRunPropertyBlock, segment.options);
                //     r++;
                // }
            }
        }
