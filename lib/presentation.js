...
let Presentation = function(args) {
	...
	if (args && args.templateFilePath && typeof args.templateFilePath === 'string') {
		...
		for (let key in powerPointStubs.fragments) {
			//console.log(`key = ${key}`);
			//console.log('fragment = ', powerPointStubs.fragments[key]);
			this.content[key] = convert.json2xml(powerPointStubs.fragments[key], { compact: false });
			//console.log('value = ', this.content[key]);
		}
	}
};
	...
	for (let key in content) {
		if (content.hasOwnProperty(key)) {
			...
			if (ext === '.xml' || ext === '.rels') {
				...
				if(this.templateFilePath != null) { // if we're coming from a file, we'll have XML in this.content instead of JSON, so convert...
