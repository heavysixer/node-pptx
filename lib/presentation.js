...
class Presentation {
	...
	addSlide(slideName, layoutName) {
		if (!slideName) throw new Error('A slide name must be specified when calling Presentation.addSlide().');
		return this.powerPointFactory.addSlide(slideName || '', layoutName || 'slideLayout1');
	}
