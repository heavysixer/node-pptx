...
class Presentation {
    ...
    async addSlide(config, layoutName) {
        ...
        if (arguments.length === 0) return this.powerPointFactory.addSlide(this, layoutName || this.defaultSlideLayout);

        // in this case the config param will be the layout name (but use defaultSlideLayout if it's blank)
        if (arguments.length === 1 && typeof config === 'string') return this.powerPointFactory.addSlide(this, config || this.defaultSlideLayout);

        // if there is a composition function, call it, and return the presentation object
        await config(this.powerPointFactory.addSlide(this, layoutName || this.defaultSlideLayout));
