...
class Slide {
    ...
    addChart(data, options = {}) {
        try {
            let newChartObject = this.powerPointFactory.addChart(this, data, options);
    ...
    getShapeRawContent(index) {
        ...
        for (let i = 0; i < shapeNodes.length; i++) {
            ...
            console.log('content = ', currentShape['p:nvSpPr'][0]['p:cNvSpPr'][0]);

            if (currentShape['p:nvSpPr'] && currentShape['p:nvSpPr'][0]['p:cNvSpPr'] && currentShape['p:nvSpPr'][0]['p:cNvSpPr'][0]['$']) {
                console.log('last node = ', currentShape['p:nvSpPr'][0]['p:cNvSpPr'][0]['$']);

                let txBoxNode = currentShape['p:nvSpPr'][0]['p:cNvSpPr'][0]['$'].txBox;
                console.log('txBoxNode = ', txBoxNode);
                if (txBoxNode !== '1') {
                    console.log(`shape node #${i} = `, currentShape);
                    if (c++ === index) {
                        return currentShape;
                    }
                } else {
