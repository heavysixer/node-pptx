...
class Slide {
    ...
    moveTo(slideNumber) {
        try {
            this.powerPointFactory.swapSlides(this.name, `slide${slideNumber}`);
            //this.name = `slide${slideNumber}`;
            console.log('slides = ', this.powerPointFactory.slides);
        } catch (err) {
            throw new Error(`Failed to move slide to new position #: ${slideNumber}. Exception info: ${err.message}`);
        }
    }
